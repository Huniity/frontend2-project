// console.log("🔥 GENERATE TRIP ROUTE LOADED");
...
//         const { destination, numberOfDays, tripType, budgetLevel, numberOfPersons } = body as {
//             destination: string;
//             numberOfDays: number;
//             tripType: TripType;
...
//             destination, numberOfDays, tripType, budgetLevel, numberOfPersons,
//         });
        
//         // Create trip immediately with GENERATING status
...
//             model: limits.model,
//             contents: prompt,
//             config: { maxOutputTokens: limits.max_tokens },
//         });
...
//                     aiPrompt: JSON.stringify(parsed.aiTips ?? []), // reuse aiPrompt to store tips
//                     status: "DRAFT",
...
//                         // store narrative in the notes field
...
//                             // store lat/lng in notes as JSON
//                             notes: activity.lat ? JSON.stringify({ lat: activity.lat, lng: activity.lng }) : null,
//                         },
//                     });
//                 }
//             }
...
//     } catch (error: any) {
//         console.error("[/api/generate-trip] FULL ERROR:", JSON.stringify(error, null, 2));
//         console.error("[/api/generate-trip] MESSAGE:", error?.message);
//         console.error("[/api/generate-trip] CODE:", error?.code);
