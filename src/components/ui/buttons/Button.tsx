...
const SubButton = ({ name, color, width, height, textSize, textColor, children }: ButtonProps) => {
    return (
        <button style={{ backgroundColor: color, width, height, fontSize: textSize, color: textColor, textAlign: 'center', display: 'flex', alignItems: 'center', justifyContent: 'center' }} className="px-4 py-2 text-black font-bold font-made-outer-alt rounded-xl hover:bg-blue-600 transition-colors duration-300">
            {children}
            {name}
        </button>
    )
